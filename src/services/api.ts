...
export const apiService = {
  ...
  deleteAllAdminData: async () => {
    try {
      const response = await api.delete('/admin/delete-all-data');
      return response.data;
    } catch (error: any) {
      console.log('Admin delete all error:', error);
      throw new Error(error.response?.data?.message || error.message || 'Failed to delete all data.');
    }
  },

  fixIncompleteSessions: async () => {
    try {
      const response = await api.post('/admin/fix-sessions');
      return response.data;
    } catch (error: any) {
      console.log('Admin fix sessions error:', error);
      throw new Error(error.response?.data?.message || error.message || 'Failed to fix sessions.');
    }
  },
